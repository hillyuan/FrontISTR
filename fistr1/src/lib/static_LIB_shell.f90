...
contains
  ...
  subroutine STF_Shell_MITC1                                          &
    ...
    real(kind = kreal), intent(in), optional :: nddisp(:, :)
    ...
    integer :: npoints_tying(3)
    integer :: it, ip
    integer :: na, nb, ng, cng
    integer :: isize, jsize
    integer :: jsize1, jsize2, jsize3, jsize4, jsize5, jsize6
    integer :: n_layer,n_totlyr, sstable(24)

    real(kind = kreal) :: D(5, 5), B(5, ndof*nn), DB(5, ndof*nn), mat66(6,6)
    real(kind = kreal) :: tmpstiff(ndof*nn, ndof*nn)
    real(kind = kreal) :: elem(3, nn)
    real(kind = kreal) :: xi_lx, eta_lx, zeta_ly
    real(kind = kreal) :: w_w_lx, w_ly
    ...
    real(kind = kreal) :: stress(6), BN(9,NDOF*NN), SBN(9,NDOF*NN),Smat(9,9)
    real(kind = kreal) :: naturalcoord(2)
    real(kind = kreal) :: tpcoord(6, 2, 3)
    real(kind = kreal) :: nncoord(nn, 2)
    real(kind = kreal) :: shapefunc(nn)
    ...
    real(kind = kreal) :: aa1(3), aa2(3), aa3(3)
    real(kind = kreal) :: bb1(3), bb2(3), bb3(3)
    real(kind = kreal) :: cc1(3), cc2(3), cc3(3)
    real(kind = kreal) :: alpha
    real(kind = kreal) :: xxi_lx, eeta_lx
    real(kind = kreal) :: xxi_di(6, 3), eeta_di(6, 3)
    real(kind = kreal) :: h(nn, 3)
    ...
    real(kind = kreal) :: u_rot(3, nn)
    real(kind = kreal) :: dudxi_rot(3, nn), dudeta_rot(3, nn), dudzeta_rot(3, nn)
    real(kind = kreal) :: g1(3), g2(3), g3(3),inverse(3,3), trans(3,3), coordsys(3,3)
    real(kind = kreal) :: g3_abs
    real(kind = kreal) :: e_0(3)
    real(kind = kreal) :: cg1(3), cg2(3), cg3(3), geomat(5,5)
    real(kind = kreal) :: det, outa(2)
    real(kind = kreal) :: det_cg3(3)
    real(kind = kreal) :: det_inv
    real(kind = kreal) :: det_cg3_abs
    real(kind = kreal) :: w_w_w_det
    ...
    real(kind = kreal) :: e1_hat_abs, e2_hat_abs
    real(kind = kreal) :: Cv12(ndof*nn), Cv13(ndof*nn), &
      Cv21(ndof*nn), Cv23(ndof*nn), Cv31(ndof*nn), Cv32(ndof*nn)
    real(kind = kreal) :: Cv_theta(ndof*nn), Cv_w(ndof*nn)
    real(kind = kreal) :: Cv(ndof*nn)
    real(kind = kreal) :: sumlyr
    real(kind = kreal), allocatable :: xabsc(:), weig(:)
    logical :: ierr
    ...
    if( gausses(1)%pMaterial%integpoints>0 ) ny=gausses(1)%pMaterial%integpoints
    ng = NumOfQuadPoints(fetype,ny)/ny
    allocate( xabsc(ny), weig(ny) )
    call gauleg(ny, xabsc(1:ny), weig(1:ny))
    ...
    elem(:, :) = ecoord(:, :)
    if( flag == UPDATELAG ) elem(:,:) = ecoord(:,:) + nddisp(:, :)
    ...
    do n_layer=1,n_totlyr
      do ly = 1, ny
          ...
          do ip = 1, npoints_tying(it)
            ...
            do na = 1, nn
              ...
              do i = 1, 3
                ...
                u_rot(i, na)                      &
                  = shapefunc(na)*( zeta_ly*a_over_2_v3(i, na) )

                dudxi_rot(i, na)                  &
                  = shapederiv(na, 1)*( zeta_ly*a_over_2_v3(i, na) )
                dudeta_rot(i, na)                 &
                  = shapederiv(na, 2)*( zeta_ly*a_over_2_v3(i, na) )
                dudzeta_rot(i, na)                &
                  = shapefunc(na)*( a_over_2_v3(i, na) )
            ...
            do nb = 1, nn
              ...
              B_di(1, jsize1, ip, it) = shapederiv(nb, 1)*g1(1)
              B_di(2, jsize1, ip, it) = shapederiv(nb, 2)*g2(1)
              B_di(3, jsize1, ip, it) = shapederiv(nb, 1)*g2(1) &
                +shapederiv(nb, 2)*g1(1)
              B_di(4, jsize1, ip, it) = shapederiv(nb, 2)*g3(1)
              B_di(5, jsize1, ip, it) = shapederiv(nb, 1)*g3(1)

              B_di(1, jsize2, ip, it) = shapederiv(nb, 1)*g1(2)
              B_di(2, jsize2, ip, it) = shapederiv(nb, 2)*g2(2)
              B_di(3, jsize2, ip, it) = shapederiv(nb, 1)*g2(2) &
                +shapederiv(nb, 2)*g1(2)
              B_di(4, jsize2, ip, it) = shapederiv(nb, 2)*g3(2)
              B_di(5, jsize2, ip, it) = shapederiv(nb, 1)*g3(2)

              B_di(1, jsize3, ip, it) = shapederiv(nb, 1)*g1(3)
              B_di(2, jsize3, ip, it) = shapederiv(nb, 2)*g2(3)
              B_di(3, jsize3, ip, it) = shapederiv(nb, 1)*g2(3) &
                +shapederiv(nb, 2)*g1(3)
              B_di(4, jsize3, ip, it) = shapederiv(nb, 2)*g3(3)
              B_di(5, jsize3, ip, it) = shapederiv(nb, 1)*g3(3)

              B_di(1, jsize4, ip, it) = aa1(1)
              B_di(2, jsize4, ip, it) = bb2(1)
              B_di(3, jsize4, ip, it) = aa2(1)+bb1(1)
              B_di(4, jsize4, ip, it) = bb3(1)+cc2(1)
              B_di(5, jsize4, ip, it) = aa3(1)+cc1(1)

              B_di(1, jsize5, ip, it) = aa1(2)
              B_di(2, jsize5, ip, it) = bb2(2)
              B_di(3, jsize5, ip, it) = aa2(2)+bb1(2)
              B_di(4, jsize5, ip, it) = bb3(2)+cc2(2)
              B_di(5, jsize5, ip, it) = aa3(2)+cc1(2)

              B_di(1, jsize6, ip, it) = aa1(3)
              B_di(2, jsize6, ip, it) = bb2(3)
              B_di(3, jsize6, ip, it) = aa2(3)+bb1(3)
              B_di(4, jsize6, ip, it) = bb3(3)+cc2(3)
              B_di(5, jsize6, ip, it) = aa3(3)+cc1(3)
        ...
        do m = 1, n_layer
          sumlyr = sumlyr +2 *gausses(1)%pMaterial%shell_var(m)%weight
        enddo
        zeta_ly = -1 +sumlyr -gausses(1)%pMaterial%shell_var(n_layer)%weight*(1-xabsc(ly))

        w_ly    = weig(ly)    !wgt(ny, ly)
        ...
        do lx = 1, ng
          ...
          do nb = 1, nn
            ...
            cc3(1) = dudzeta_rot(2, nb)*g3(3)-dudzeta_rot(3, nb)*g3(2)
            cc3(2) = dudzeta_rot(3, nb)*g3(1)-dudzeta_rot(1, nb)*g3(3)
            cc3(3) = dudzeta_rot(1, nb)*g3(2)-dudzeta_rot(2, nb)*g3(1)

            B(1, jsize1) = shapederiv(nb, 1)*g1(1)
            B(2, jsize1) = shapederiv(nb, 2)*g2(1)
            B(3, jsize1) = shapederiv(nb, 1)*g2(1)+shapederiv(nb, 2)*g1(1)
            B(4, jsize1) = shapederiv(nb, 2)*g3(1)
            B(5, jsize1) = shapederiv(nb, 1)*g3(1)

            B(1, jsize2) = shapederiv(nb, 1)*g1(2)
            B(2, jsize2) = shapederiv(nb, 2)*g2(2)
            B(3, jsize2) = shapederiv(nb, 1)*g2(2)+shapederiv(nb, 2)*g1(2)
            B(4, jsize2) = shapederiv(nb, 2)*g3(2)
            B(5, jsize2) = shapederiv(nb, 1)*g3(2)

            B(1, jsize3) = shapederiv(nb, 1)*g1(3)
            B(2, jsize3) = shapederiv(nb, 2)*g2(3)
            B(3, jsize3) = shapederiv(nb, 1)*g2(3)+shapederiv(nb, 2)*g1(3)
            B(4, jsize3) = shapederiv(nb, 2)*g3(3)
            B(5, jsize3) = shapederiv(nb, 1)*g3(3)
            ...
            IF( flag == UPDATELAG ) then
              BN(1, jsize1) = shapederiv(nb, 1)*g1(1)
              BN(1, jsize2) = shapederiv(nb, 1)*g1(2)
              BN(1, jsize3) = shapederiv(nb, 1)*g1(3)
              BN(1, jsize4) = aa1(1)
              BN(1, jsize5) = aa1(2)
              BN(1, jsize6) = aa1(3)

              BN(2, jsize1) = shapederiv(nb, 2)*g1(1)
              BN(2, jsize2) = shapederiv(nb, 2)*g1(2)
              BN(2, jsize3) = shapederiv(nb, 2)*g1(3)
              BN(2, jsize4) = bb1(1)
              BN(2, jsize5) = bb1(2)
              BN(2, jsize6) = bb1(3)
              ...
              BN(4, jsize1) = shapederiv(nb, 1)*g2(1)
              BN(4, jsize2) = shapederiv(nb, 1)*g2(2)
              BN(4, jsize3) = shapederiv(nb, 1)*g2(3)
              BN(4, jsize4) = aa2(1)
              BN(4, jsize5) = aa2(2)
              BN(4, jsize6) = aa2(3)

              BN(5, jsize1) = shapederiv(nb, 2)*g2(1)
              BN(5, jsize2) = shapederiv(nb, 2)*g2(2)
              BN(5, jsize3) = shapederiv(nb, 2)*g2(3)
              BN(5, jsize4) = bb2(1)
              BN(5, jsize5) = bb2(2)
              BN(5, jsize6) = bb2(3)
              ...
              BN(7, jsize1) = shapederiv(nb, 1)*g3(1)
              BN(7, jsize2) = shapederiv(nb, 1)*g3(2)
              BN(7, jsize3) = shapederiv(nb, 1)*g3(3)
              BN(7, jsize4) = aa3(1)
              BN(7, jsize5) = aa3(2)
              BN(7, jsize6) = aa3(3)

              BN(8, jsize1) = shapederiv(nb, 2)*g3(1)
              BN(8, jsize2) = shapederiv(nb, 2)*g3(2)
              BN(8, jsize3) = shapederiv(nb, 2)*g3(3)
              BN(8, jsize4) = bb3(1)
              BN(8, jsize5) = bb3(2)
              BN(8, jsize6) = bb3(3)
          ...
          if( etype .EQ. fe_mitc4_shell ) then
            ...
            do jsize = 1, ndof*nn
              ...
              B(4, jsize)                                       &
                = 0.5D0*( 1.0D0-xi_lx  )*B_di(4, jsize, 4, 1) &
                +0.5D0*( 1.0D0+xi_lx  )*B_di(4, jsize, 2, 1)
              B(5, jsize)                                       &
                = 0.5D0*( 1.0D0-eta_lx )*B_di(5, jsize, 1, 1) &
                +0.5D0*( 1.0D0+eta_lx )*B_di(5, jsize, 3, 1)
            ...
            do jsize = 1, ndof*nn
              ...
              do ip = 1, npoints_tying(1)
                ...
                ! B_as(1, jsize)
                B(1, jsize)                                      &
                  = B(1, jsize)+h(ip, 1)*B_di(1, jsize, ip, 1)
                ! B_as(5, jsize)
                B(5, jsize)                                      &
                  = B(5, jsize)+h(ip, 1)*B_di(5, jsize, ip, 1)
              ...
              do ip = 1, npoints_tying(2)
                ...
                ! B_as(2, jsize)
                B(2, jsize)                                      &
                  = B(2, jsize)+h(ip, 2)*B_di(2, jsize, ip, 2)
                ! B_as(4, jsize)
                B(4, jsize)                                      &
                  = B(4, jsize)+h(ip, 2)*B_di(4, jsize, ip, 2)
              ...
              do ip = 1, npoints_tying(3)
                ...
                ! B_as(3, jsize)
                B(3, jsize)                                      &
                  = B(3, jsize)+h(ip, 3)*B_di(3, jsize, ip, 3)
            ...
            do jsize = 1, ndof*nn
              ...
              B(4, jsize)                                 &
                = ( 1.0D0-xi_lx  )*B_di(4, jsize, 2, 1) &
                +xi_lx *B_di(5, jsize, 1, 1)           &
                +xi_lx *( B_di(4, jsize, 3, 1)         &
                -B_di(5, jsize, 3, 1)  )

              B(5, jsize)                                 &
                = eta_lx*B_di(4, jsize, 2, 1)           &
                +( 1.0D0-eta_lx )*B_di(5, jsize, 1, 1) &
                -eta_lx*( B_di(4, jsize, 3, 1)         &
                -B_di(5, jsize, 3, 1)  )
          ...
          DB(:, 1:ndof*nn) = matmul( D, B(:, 1:ndof*nn ) )
          ...
          IF( flag == UPDATELAG ) then
            ...
            BN(1:3,:) = matmul(trans,BN(1:3,:) )
            BN(4:6,:) = matmul(trans,BN(4:6,:) )
            BN(7:9,:) = matmul(trans,BN(7:9,:) )

            Smat(:,:) = 0.d0
            Smat(1,1) = stress(1); Smat(1,2) = stress(4); Smat(1,3) = stress(6)
            Smat(2,1) = stress(4); Smat(2,2) = stress(2); Smat(2,3) = stress(5)
            Smat(3,1) = stress(6); Smat(3,2) = stress(5); Smat(3,3) = 0.d0
            Smat(4:6,4:6) = Smat(1:3,1:3)
            Smat(7:9,7:9) = Smat(1:3,1:3)
            SBN(:,1:nn*ndof) = matmul( Smat, BN(:,1:nn*ndof) )

            forall( i=1:nn*ndof, j=1:nn*ndof )
              stiff(i,j) = stiff(i,j)+dot_product( BN(:,i), SBN(:,j) )*w_w_w_det
            end forall
          ENDIF
    ...
    deallocate( xabsc, weig )

  !####################################################################
  end subroutine STF_Shell_MITC1
  !####################################################################
  
  !####################################################################
  subroutine Update_Shell_MITC                                           &
      (etype, nn, ndof, ecoord, unode, du, gausses, &
      qf, thick)
  !####################################################################
    ...
    integer(kind = kint), intent(in) :: ndof
    real(kind = kreal), intent(in)   :: ecoord(:, :)
    real(kind = kreal), intent(in)   :: unode(:, :)
    real(kind = kreal), intent(in)   :: du(:, :)
    ...
    real(kind = kreal) :: D(5, 5), edisp(6, nn), B(5, ndof*nn)
    real(kind = kreal) :: elem(3, nn),  B_di(5, ndof*nn, 6, 3)
    real(kind = kreal) :: xi_lx, eta_lx, zeta_ly
    real(kind = kreal) :: naturalcoord(2)
    real(kind = kreal) :: tpcoord(6, 2, 3)
    real(kind = kreal) :: nncoord(nn, 2)
    ...
    real(kind = kreal) :: dudxi(3), dudeta(3), dudzeta(3)
    real(kind = kreal) :: dudxi_rot(3, nn), dudeta_rot(3, nn), dudzeta_rot(3, nn)
    real(kind = kreal) :: g1(3), g2(3), g3(3),trans(3,3), inverse(3,3), Q(6,6)
    real(kind = kreal) :: g3_abs
    real(kind = kreal) :: e_0(3)
    real(kind = kreal) :: cg1(3), cg2(3), cg3(3), dstrain(6), dstress(6)
    real(kind = kreal) :: det, w_w_w_det, w_ly, w_w_lx
    real(kind = kreal) :: det_cg3(3)
    real(kind = kreal) :: det_inv
    real(kind = kreal) :: det_cg3_abs
    real(kind = kreal) :: e1_hat(3), e2_hat(3), e3_hat(3)
    real(kind = kreal) :: e1_hat_abs, e2_hat_abs
    real(kind = kreal) :: e11, e22, e33, e12_2, e23_2, e31_2
    real(kind = kreal) :: e11_di(6, 3), e22_di(6, 3),     &
      e12_di_2(6, 3), e23_di_2(6, 3), e31_di_2(6, 3)
    real(kind = kreal) :: S(3, 3), Sv(5), Ev(5), E(3,3)
    real(kind = kreal) :: sumlyr, coordsys(3,3), F(3,3)
    integer :: isize, jsize, jsize1, jsize2, jsize3, jsize4, jsize5, jsize6
    real(kind = kreal) :: aa1(3), aa2(3), aa3(3)
    real(kind = kreal) :: bb1(3), bb2(3), bb3(3)
    real(kind = kreal) :: cc1(3), cc2(3)
    real(kind = kreal), allocatable :: xabsc(:), weig(:)
	real(kind = kreal) :: rot(3,3) = reshape((/ 1.d0, 0.d0, 0.d0, 0.d0, 1.d0, 0.d0, 0.d0, 0.d0, 1.d0 /), shape(rot))
      ...
      do ip = 1, npoints_tying(it)
        ...
        do i = 1, 3
          ...
          do na = 1, nn
            ...
            dudxi(i)                                      &
              = dudxi(i)                                    &
              +shapederiv(na, 1)*( edisp(i, na)                             &
              +zeta_ly*a_over_2_theta_cross_v3(i, na) )
            dudeta(i)                                     &
              = dudeta(i)                                   &
              +shapederiv(na, 2)*( edisp(i, na)                             &
              +zeta_ly*a_over_2_theta_cross_v3(i, na) )
            dudzeta(i)                                    &
              = dudzeta(i)                                  &
              +shapefunc(na)*( a_over_2_theta_cross_v3(i, na) )
    ...
    do lx = 1,ng 
      ...
      e33                                       &
        = 0.5D0                                    &
        *( ( g3(1)*dudzeta(1)+dudzeta(1)*g3(1) )   &
        +( g3(2)*dudzeta(2)+dudzeta(2)*g3(2) )   &
        +( g3(3)*dudzeta(3)+dudzeta(3)*g3(3) ) )
      e12_2                                   &
        = ( g1(1)*dudeta(1) +dudxi(1)  *g2(1) ) &
        +( g1(2)*dudeta(2) +dudxi(2)  *g2(2) ) &
        +( g1(3)*dudeta(3) +dudxi(3)  *g2(3) )
      ...
      !--------------------------------------------------
	!  if( flag == UPDATELAG ) then
      if( isElastoplastic(gausses(cng)%pMaterial%mtype) ) then
        coordsys(1,:) = e1_hat
        coordsys(2,:) = e2_hat
        coordsys(3,:) = e3_hat
        inverse(:,1) = cg1
        ...
        dstrain(5) = e23_2
        dstrain(6) = e31_2
        dstrain = matmul( Q, dstrain(:) )          ! into local orthogonal coordinate system
        call UpdateStress_Shell( flag, gausses(cng), dstrain, rot, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0 )
        dstress = matmul( gausses(cng)%stress(:), Q )    ! transfer stress into covariant frame
        Sv(1:2) = dstress(1:2); Sv(3:5) = dstress(4:6)
      else
        call MatlMatrix_Shell(gausses(cng), Shell, D,                 &
          e1_hat, e2_hat, e3_hat, cg1, cg2, cg3, &
          alpha, n_layer)
        ...
        dstress(6)                 &
          ...
          +S(3, 1)*g3(3)*g1(1)   &
          +S(3, 2)*g3(3)*g2(1) )
        gausses(cng)%stress_out = dstress
	  ...
	  ! [ B L ] matrix
        do nb = 1, nn
          ...
          w_w_w_det = w_w_lx*w_ly*det
        !--------------------------------------------------
          qf = qf + matmul( Sv, B )*w_w_w_det
        enddo
        !--------------------------------------------------
        
      end do     !< LAMINATED SHELL ANALYSIS

      if(allocated(xabsc)) deallocate( xabsc, weig )
    end do
  ...
  subroutine UpdateStress_Shell( flag, gauss, dstrain, coordsys, time, tincr, ttc, tt0, ttn )
    use m_fstr
    use m_MatMatrix
    use mMechGauss
    use m_utilities
    ...
    real(kind=kreal), intent(inout)         :: dstrain(6)    ! strain in local orthogonal coordinate system
    ...
    real(kind=kreal)   :: D(6,6), dstress(6), dumstress(3,3), dum(3,3), det
    real(kind=kreal)   :: tensor(6)     !< tensor
    real(kind=kreal)   :: st33, eigval(3)

    mtype = gauss%pMaterial%mtype

    call MatlMatrix( gauss, Shell, D, time, tincr, coordsys, ttc, 0 )
    st33 = dot_product( D(3,:), dstrain ) /D(3,3)
    dstrain(3) = -st33
    ...
    if( flag == INFINITESIMAL ) then
      ...
      gauss%stress(:) = gauss%stress_bak(:) + dstress(:)

      if( isElastoplastic(mtype) ) then
        call MatlMatrix( gauss, Shell, D, time, tincr, coordsys, ttc, 1 )
        call BackwardEuler_Shell( gauss%pMaterial, D, dstrain, gauss%stress, gauss%stress_bak, gauss%plstrain, &
